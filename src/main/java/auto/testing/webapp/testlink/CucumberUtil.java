package auto.testing.webapp.testlink;
...
public class CucumberUtil {
	...
	public static String addCucumberString(String value) {
		...
		for (int i = 1; i < tempString.length; i++) {
			...
			if (temp.startsWith("#")) {
				...
				tempString[i] = "\tWhen " + tempString[i].trim();
			} else if ((!temp.startsWith("when")) && (!temp.startsWith("and")) && (!temp.contains("given"))
					&& (!temp.contains("scenario")) && (!temp.startsWith("then")) && (!temp.contains("background"))
					&& (!temp.contains("feature")) && (!temp.startsWith("example")) && (!temp.startsWith("|"))) {
	...
	public static String processSpecialString(String value) {
		String retString = "";
		retString = value.replaceAll("<p>", "").replaceAll("</p>", "").replaceAll("<ol>", "").replaceAll("</ol>", "")
				.replaceAll("<li>", "").replaceAll("</li>", "").replaceAll("<br>", "").replaceAll("<br />", "")
				.replaceAll("</br>", "")
				// .replaceAll(";", "")
				.replaceAll("\";", "\"").replaceAll("\";", "\"").replaceAll("&nbsp", "").replaceAll("&quot", "\"")
				.replaceAll("<div>", "").replaceAll("</div>", "").replaceAll("<strong>", "").replaceAll("</strong>", "")
				...
				// .replaceAll("/span", "")
				.replaceAll("&#39", "'").replaceAll("&gt", ">").replaceAll("&lt", "<").replaceAll("<>", " ")
				.replaceAll("<file>", "").replaceAll("</file>", "").replace(" /", "/").replace("  /", "/")
				.replace("&amp", "&").replace("When When", "When").replace("When Then", "Then")
				.replace("When And", "And").replace("When Background", "Background").replaceAll("[\\\r\\\n]+", "\n")
				.replaceAll("[\n\r]+", "\n");
		return retString;
	}
